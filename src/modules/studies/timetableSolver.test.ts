...
describe("moteur déterministe d’horaires",()=>{
  ...
  it.each([
    ...
    expect(result.success).toBe(true);
    const groups = [...new Set(result.entries.map((entry) => entry.blockId))].map((blockId) => result.entries.filter((entry) => entry.blockId === blockId));
  ...
  it("place un bloc de six à travers une pause sans occuper la pause",()=>{const configured={...assignment("a","t","c","m",6),sessionPattern:{mode:"blocks" as const,blocks:[6]}};const result=solver.solve(problem([configured],[period("p1",1),period("p2",2),period("p3",3),period("pause",4,"break"),period("p4",5),period("p5",6),period("p6",7)]));expect(result.success).toBe(true);expect(result.entries).toHaveLength(6);expect(result.entries.some((entry)=>entry.periodId==="pause")).toBe(false)});
  it("refuse un bloc supérieur à la limite métier de six",()=>{const configured={...assignment("a","t","c","m",7),sessionPattern:{mode:"blocks" as const,blocks:[7]}};const result=solver.solve(problem([configured],Array.from({length:7},(_,index)=>period(`p${index+1}`,index+1))));expect(result.success).toBe(false);expect(result.failures[0].reason).toContain("6 périodes consécutives")});
  ...
  it("place un bloc de quatre à travers une pause",()=>{const configured={...assignment("a","t","c","m",4),sessionPattern:{mode:"blocks" as const,blocks:[4]}};const result=solver.solve(problem([configured],[period("p1",1),period("p2",2),period("pause",3,"break"),period("p3",4),period("p4",5)]));expect(result.success).toBe(true);expect(result.entries).toHaveLength(4);expect(result.entries.some((entry)=>entry.periodId==="pause")).toBe(false)});
  it("place un bloc de six à travers deux pauses officielles",()=>{const configured={...assignment("a","t","c","m",6),sessionPattern:{mode:"blocks" as const,blocks:[6]}};const periods=[period("p1",1),period("p2",2),period("pause-1",3,"break"),period("p3",4),period("p4",5),period("pause-2",6,"recess"),period("p5",7),period("p6",8)];const result=solver.solve(problem([configured],periods));expect(result.success).toBe(true);expect(result.entries).toHaveLength(6);expect(result.entries.every((entry)=>periods.find((item)=>item.id===entry.periodId)?.type==="course")).toBe(true)});
  it.each([
    { weeklyPeriods: 4, blocks: [4] },
    { weeklyPeriods: 5, blocks: [5] },
    { weeklyPeriods: 6, blocks: [6] },
    ...
    { weeklyPeriods: 8, blocks: [6, 2] },
    { weeklyPeriods: 9, blocks: [2, 3, 4] },
  ])("place les blocs $blocks à travers une récréation sans l’occuper",({weeklyPeriods,blocks})=>{const configured={...assignment("pause-blocks","t","c","m",weeklyPeriods),sessionPattern:{mode:"blocks" as const,blocks}};const periods=[period("p1",1),period("p2",2),period("p3",3),period("recreation",4,"recess"),period("p4",5),period("p5",6),period("p6",7)];const result=solver.solve(problem([configured],periods));expect(result.success).toBe(true);expect(result.entries).toHaveLength(weeklyPeriods);expect(result.entries.some((entry)=>entry.periodId==="recreation")).toBe(false);const groups=[...new Set(result.entries.map((entry)=>entry.blockId))].map((blockId)=>result.entries.filter((entry)=>entry.blockId===blockId));expect(groups.map((group)=>group.length).sort((left,right)=>left-right)).toEqual([...blocks].sort((left,right)=>left-right))});
  ...
  it("transmet au solveur les vacations indépendantes des options opérationnelles",()=>{const morning={...period("morning",1),vacation:"morning" as const,dayScope:"weekdays" as const};const afternoon={...period("afternoon",2),vacation:"afternoon" as const,dayScope:"weekdays" as const};const parent=schoolClass("3h","morning");const science={...schoolClass("3h::sci","afternoon"),parentClassId:"3h",classOptionKey:"3h::sci",option:"Sciences"};const literary={...schoolClass("3h::lit","morning"),parentClassId:"3h",classOptionKey:"3h::lit",option:"Littéraire"};const scoped=(id:string,teacher:string,target:string)=>({...assignment(id,teacher,"3h",id),courseScope:"option" as const,targetOptionIds:[target],studentGroupKey:`option--${target}`});const input={...problem([scoped("science","t1",science.id),scoped("literary","t2",literary.id)],[morning,afternoon]),classes:[parent,science,literary],days:["monday" as const]};const result=solver.solve(input);expect(result.success).toBe(true);expect(result.entries.find((item)=>item.assignmentId==="science")?.periodId).toBe("afternoon");expect(result.entries.find((item)=>item.assignmentId==="literary")?.periodId).toBe("morning");expect(diagnoseAssignmentSlots(input.assignments[0],input).classCompatibleSlots).toBe(1)});
  it("place un cours double à travers une pause sans occuper la pause",()=>{const result=solver.solve(problem([assignment("a","t","c","m",2,2)],[period("p1",1),period("pause",2,"break"),period("p2",3)]));expect(result.success).toBe(true);expect(result.entries.map((entry)=>entry.periodId).sort()).toEqual(["p1","p2"])});
  ...
  it("résout sans explosion combinatoire un graphe dense de 260 périodes",()=>{
    ...
    expect(result.statistics.backtracks).toBe(0);
    expect(result.statistics.maxDepth).toBe(260);
  });
