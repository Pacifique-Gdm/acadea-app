...
describe("layout partagé de la Boîte à Messagerie", () => {
  ...
  it("reste aligné à droite, pleine hauteur et responsive avec le Drawer Administrateur", () => {
    ...
    expect(drawerSource).toContain("box-border ml-auto flex h-full min-h-0 w-full min-w-0 max-w-[calc(100vw-1.5rem)] flex-col rounded border border-slate-200 bg-white p-4 shadow-2xl");
    expect(drawerSource).toContain(': "sm:max-w-xl"');
