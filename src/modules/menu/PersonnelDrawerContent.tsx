...
export function PersonnelDrawerContent({ user, school }: { user: AppUser; school: School }) {
  ...
  return <div className="grid gap-4">
    ...
    {selected && <AdminDrawer title={`Personnel — ${selected.name}`} closeLabel="Fermer la fiche Personnel" onClose={() => !busy && setSelected(undefined)}>
      {selected.role !== "school_admin" && <div className="flex flex-wrap justify-end gap-2"><button type="button" className="secondary-button" disabled={busy} onClick={() => openEdit(selected)}><Pencil className="h-4 w-4"/> Modifier</button>{isArchivedPersonnel(selected) ? <button type="button" className="primary-button" disabled={busy} onClick={() => setConfirming("reactivate")}><ArchiveRestore className="h-4 w-4"/> Réactiver</button> : <button type="button" className="rounded bg-red-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50" disabled={busy} onClick={() => setConfirming("archive")}><Archive className="inline h-4 w-4"/> Archiver</button>}</div>}
