...
import { operationalClassLabel, periodAppliesToAssignment, periodAppliesToClass, studyClassSection, subjectAppliesToClass } from "./studyScope";
import type { PedagogicalAssignment, SchedulePeriod, StudyClass, StudySubject } from "./studyTypes";
...
describe("périmètre pédagogique section/classe/vacation",()=>{
 ...
 it("normalise une classe historique sans vacation comme classe du matin",()=>{const legacy={...baseClass,vacation:undefined};expect(periodAppliesToClass(period({vacation:"morning"}),legacy,"monday")).toBe(true);expect(periodAppliesToClass(period({vacation:"afternoon"}),legacy,"monday")).toBe(false)});
 it("applique indépendamment les vacations des options ciblées",()=>{const parent={...baseClass,id:"3h",option:undefined,vacation:"morning" as const};const science={...baseClass,id:"3h::science",parentClassId:"3h",classOptionKey:"3h::science",option:"Sciences",vacation:"afternoon" as const};const literary={...baseClass,id:"3h::literary",parentClassId:"3h",classOptionKey:"3h::literary",option:"Littéraire",vacation:"morning" as const};const scoped=(targetOptionIds:string[]):PedagogicalAssignment=>({id:"a",schoolId:"s",schoolYearId:"y",teacherId:"t",subjectId:"m",classId:"3h",weeklyPeriods:1,active:true,courseScope:targetOptionIds.length===1?"option":"common",targetOptionIds,createdBy:"u",updatedBy:"u",createdAt:"n",updatedAt:"n"});expect(periodAppliesToAssignment(period({vacation:"afternoon"}),scoped([science.id]),[parent,science,literary],"monday")).toBe(true);expect(periodAppliesToAssignment(period({vacation:"morning"}),scoped([science.id]),[parent,science,literary],"monday")).toBe(false);expect(periodAppliesToAssignment(period({vacation:"morning"}),scoped([literary.id]),[parent,science,literary],"monday")).toBe(true);expect(periodAppliesToAssignment(period({vacation:"morning"}),scoped([science.id,literary.id]),[parent,science,literary],"monday")).toBe(false)});
});
