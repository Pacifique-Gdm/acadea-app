...
export function SubCoordinationManagement({ coordination, schools, currentUser }: Props) {
  ...
  return <div className="grid min-w-0 gap-4">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded border border-blue-100 bg-blue-50 p-4"><div><h3 className="font-bold">Sous-coordinations</h3><p className="text-sm text-slate-600">Déléguez la supervision sans transférer l’autorité annuelle.</p></div><button type="button" className="primary-button justify-center" onClick={() => { resetForm(); setCreateOpen(true); }}><Plus className="h-4 w-4"/>Créer sous-coordination</button></div>
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]"><div className="grid content-start gap-2">{items.map((item) => <button key={item.id} type="button" className={`flex items-center justify-between gap-2 rounded border p-3 text-left ${selectedId === item.id ? "border-blue-400 bg-blue-50" : "bg-white"}`} onClick={() => setSelectedId(item.id)}><span className="min-w-0"><strong className="block truncate">{item.circumscription}</strong><span className="text-xs text-slate-500">{item.active ? "Active" : "Archivée"} · {relations.filter((relation) => relation.subCoordinationId === item.id && relation.active).length} école(s)</span></span><ChevronRight className="h-4 w-4 shrink-0"/></button>)}{items.length === 0 && <p className="rounded bg-slate-50 p-4 text-sm text-slate-500">Aucune Sous-coordination.</p>}</div>
      <div className="rounded border bg-white p-4">{!selected && <p className="text-sm text-slate-500">Sélectionnez une Sous-coordination pour consulter sa fiche.</p>}{selected && <div className="grid gap-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h4 className="text-lg font-bold">{selected.circumscription}</h4><p className="text-sm text-slate-500">Créée le {dateLabel(selected.createdAt)}</p></div><button type="button" disabled={busy} onClick={() => void changeStatus()} className={selected.active ? "inline-flex items-center gap-2 rounded border border-red-200 px-3 py-2 text-sm font-semibold text-red-700" : "secondary-button"}>{selected.active ? <Archive className="h-4 w-4"/> : <ArchiveRestore className="h-4 w-4"/>}{selected.active ? "Archiver" : "Réactiver"}</button></div><p className="inline-flex items-center gap-2 text-sm"><UserRound className="h-4 w-4"/><b>Sous-coordinateur :</b> {selectedCoordinator?.name ?? selected.coordinatorUserId}</p><p className="text-sm"><b>Statut :</b> {selected.active ? "Actif" : "Archivé"}</p><div><h5 className="mb-2 font-semibold">Écoles supervisées ({activeRelations.length})</h5><div className="grid gap-2">{selectedRelations.map((relation) => <div key={relation.id} className="flex flex-wrap items-center justify-between gap-2 rounded border p-2 text-sm"><span className="inline-flex items-center gap-2"><Building2 className="h-4 w-4 text-blue-600"/>{schools.find((school) => school.id === relation.schoolId)?.name ?? relation.schoolId}<span className={`rounded-full px-2 py-0.5 text-xs ${relation.active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{relation.active ? "Active" : `Retirée le ${dateLabel(relation.removedAt)}`}</span></span>{relation.active && <button type="button" disabled={busy} className="rounded border border-red-200 px-2 py-1 text-xs font-semibold text-red-700" onClick={() => void changeSchool(relation.schoolId, "remove")}>Retirer</button>}</div>)}</div></div>{selected.active && <div><h5 className="mb-2 font-semibold">Ajouter une école disponible</h5><div className="grid gap-2 sm:grid-cols-2">{availableSchools.map((school) => <button key={school.id} type="button" disabled={busy} className="rounded border border-blue-200 px-3 py-2 text-left text-sm text-blue-700" onClick={() => void changeSchool(school.id, "add")}>{school.name}</button>)}{availableSchools.length === 0 && <p className="text-sm text-slate-500">Aucune école disponible.</p>}</div></div>}{activeRelations.length > 0 && items.some((item) => item.id !== selected.id && item.active) && <div className="grid gap-2 rounded border p-3"><h5 className="font-semibold">Transférer une école</h5><select className="input" aria-label="École à transférer" value={transferSchoolId} onChange={(event) => setTransferSchoolId(event.target.value)}><option value="">Choisir l’école</option>{activeRelations.map((relation) => <option key={relation.schoolId} value={relation.schoolId}>{schools.find((school) => school.id === relation.schoolId)?.name ?? relation.schoolId}</option>)}</select><select className="input" aria-label="Sous-coordination cible" value={transferTargetId} onChange={(event) => setTransferTargetId(event.target.value)}><option value="">Choisir la cible</option>{items.filter((item) => item.id !== selected.id && item.active).map((item) => <option key={item.id} value={item.id}>{item.circumscription}</option>)}</select><button type="button" className="secondary-button justify-center" disabled={busy || !transferSchoolId || !transferTargetId} onClick={() => void transferSchool()}>Transférer</button></div>}</div>}</div></div>
    ...
    {createOpen && <AdminDrawer title="Créer sous-coordination" closeLabel="Fermer" onClose={() => !busy && setCreateOpen(false)}><div className="grid min-w-0 gap-3"><label className="grid gap-1 text-sm font-medium">Nom<input className="input" value={lastName} onChange={(event) => setLastName(event.target.value)}/></label><label className="grid gap-1 text-sm font-medium">Postnom<input className="input" value={middleName} onChange={(event) => setMiddleName(event.target.value)}/></label><label className="grid gap-1 text-sm font-medium">Prénom<input className="input" value={firstName} onChange={(event) => setFirstName(event.target.value)}/></label><label className="grid gap-1 text-sm font-medium">Téléphone<input className="input" value={phone} onChange={(event) => { const value = event.target.value; setPhone(value); setPassword(temporaryPasswordAfterPhoneChange({ nextPhone: value, currentPassword: password, manuallyEdited: passwordEdited })); }}/></label><label className="grid gap-1 text-sm font-medium">Email<input className="input" type="email" value={email} onChange={(event) => setEmail(event.target.value)}/></label><PasswordField label="Mot de passe temporaire" value={password} onChange={(value) => { setPassword(value); setPasswordEdited(true); }} visible={passwordVisible} onToggle={() => setPasswordVisible((value) => !value)}/><label className="grid gap-1 text-sm font-medium">Circonscription<input className="input" placeholder="Commune de Gombe" value={circumscription} onChange={(event) => setCircumscription(event.target.value)}/></label><MultiSelectDropdown label="Écoles à superviser" options={availableSchools.map((school) => ({ value: school.id, label: school.name }))} values={schoolIds} onChange={setSchoolIds} placeholder="Sélectionner les écoles"/><p className="text-sm font-semibold text-blue-700">{schoolIds.length} école{schoolIds.length > 1 ? "s" : ""} sélectionnée{schoolIds.length > 1 ? "s" : ""}</p><div className="grid grid-cols-2 gap-2"><button type="button" className="secondary-button justify-center" disabled={busy} onClick={() => setCreateOpen(false)}>Annuler</button><button type="button" className="primary-button justify-center" disabled={busy || !lastName.trim() || !phone.trim() || !email.includes("@") || password.length < 6 || !circumscription.trim()} onClick={() => void submit()}>{busy ? "Création…" : "Créer"}</button></div></div></AdminDrawer>}
  </div>;
}
