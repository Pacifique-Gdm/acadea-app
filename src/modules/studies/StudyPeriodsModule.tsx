...
export function StudyPeriodsModule({ user, school, year, data }: { user: AppUser; school: School; year: SchoolYear; data: ReturnType<typeof useStudyData> }) {
  ...
  const schoolDays = resolveAttendanceSchoolDays(data.attendanceSettings);
  const sixDays = schoolDays.includes("saturday");
  ...
  return <section className="grid gap-5">
    ...
    {editorOpen && <div className="grid gap-3 rounded border bg-white p-4"><div className="grid gap-2 sm:grid-cols-2"><select className="input" aria-label="Vacation" value={vacation} onChange={(event) => setVacation(event.target.value as StudyVacation)}><option value="morning">Avant-midi</option><option value="afternoon">Après-midi</option></select><select className="input" aria-label="Jours concernés" value={dayScope} onChange={(event) => setDayScope(event.target.value as typeof dayScope)}><option value="weekdays">Lundi à vendredi</option>{sixDays && <option value="saturday">Samedi (configuration spéciale)</option>}</select></div><input className="input" placeholder="Libellé" value={label} onChange={(event) => setLabel(event.target.value)} /><div className="grid gap-2 sm:grid-cols-2"><input className="input" aria-label="Heure de début" type="time" value={start} onChange={(event) => setStart(event.target.value)} /><input className="input" aria-label="Heure de fin" type="time" value={end} onChange={(event) => setEnd(event.target.value)} /></div><input className="input" aria-label="Ordre" type="number" min="1" value={order} onChange={(event) => setOrder(event.target.value)} /><select className="input" value={type} onChange={(event) => setType(event.target.value as SchedulePeriod["type"])}><option value="course">Cours</option><option value="break">Pause</option><option value="recess">Récréation</option></select>{error && <p role="alert" className="text-red-700">{error}</p>}<div className="grid grid-cols-2 gap-2"><button className="secondary-button justify-center" onClick={() => setEditorOpen(false)}>Annuler</button><button className="primary-button justify-center" onClick={() => void save()}>Enregistrer</button></div></div>}
  </section>;
}
