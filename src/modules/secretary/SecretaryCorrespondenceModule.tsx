...
export function SecretaryCorrespondenceModule({ user, users = [], school, year }: { user: AppUser; users?: AppUser[]; school: School; year: SchoolYear }) {
  ...
  return <section className="grid gap-4">
    ...
    <div className="grid min-w-0 gap-2 sm:grid-cols-2 lg:grid-cols-[auto_minmax(0,1fr)_minmax(0,0.8fr)_minmax(0,1.1fr)_minmax(0,0.9fr)_minmax(0,1fr)_auto]">
      ...
      <select className="input min-w-0" value={priority} onChange={(event) => setPriority(event.target.value)}><option value="all">Toutes les priorités</option><option value="normal">Normale</option><option value="important">Importante</option><option value="urgent">Urgente</option><option value="very_urgent">Très urgente</option></select>
