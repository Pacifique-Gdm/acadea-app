...
export function CoordinationStudents({ user, coordination, schools, selectedSchoolId, model, loading, loadError }: { user: AppUser; coordination: Coordination; schools: School[]; selectedSchoolId: string; model: CoordinationDashboardReadModel; loading: boolean; loadError: string }) {
  ...
  return <section className="grid min-w-0 gap-4">
    ...
    <div className="grid w-full min-w-0 grid-cols-1 items-stretch gap-2 sm:grid-cols-2 lg:flex lg:flex-nowrap">
      <label className="flex min-w-0 items-center gap-2 rounded border border-slate-200 bg-white px-3 py-2 lg:flex-1 lg:basis-0"><Search className="h-4 w-4 text-slate-400"/><input className="min-w-0 flex-1 outline-none" placeholder="Rechercher" value={search} onChange={(event) => setSearch(event.target.value)}/></label>
      <select aria-label="Statut des élèves" className="input min-w-0 lg:flex-1 lg:basis-0" value={status} onChange={(event) => setStatus(event.target.value as CoordinationStudentStatus)}><option value="all">Tous</option><option value="active">Actifs</option><option value="archived">Archivés</option></select>
      <select aria-label="Classe" className="input min-w-0 lg:flex-1 lg:basis-0" value={classKey} onChange={(event) => { setClassKey(event.target.value); setOptionKey(""); }}><option value="">Toutes les classes</option>{classes.map((choice) => <option key={choice.value} value={choice.value}>{choice.label}</option>)}</select>
      <select aria-label="Option" className="input min-w-0 lg:flex-1 lg:basis-0" value={optionKey} onChange={(event) => setOptionKey(event.target.value)}><option value="">Toutes les options</option>{options.map((choice) => <option key={choice.value} value={choice.value}>{choice.label}</option>)}</select>
      <button type="button" className="pdf-export-button min-w-0 lg:flex-1 lg:basis-0" disabled={!students.length} onClick={() => void exportPdf()}><Download className="h-4 w-4"/> Exporter PDF</button>
    </div>
