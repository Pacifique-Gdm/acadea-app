...
export function StudyDirectorPortal({ user, school, year, renderHeader, renderEnvironmentBanner, onLogout }: { user: AppUser; school: School; year: SchoolYear; renderHeader: () => ReactNode; renderEnvironmentBanner: () => ReactNode; onLogout: () => void }) {
  ...
  return <div className="flex h-screen flex-col overflow-hidden bg-[#f6f8fb]">
    ...
    <main className="mx-auto w-full max-w-7xl min-w-0 flex-1 overflow-y-auto px-3 py-5 pb-28 sm:px-6 sm:pb-32 lg:px-8">
      ...
      {activeTab === "menu" && <section className="grid gap-4"><button type="button" className="rounded border border-slate-200 bg-white p-5 text-left shadow-sm hover:bg-slate-50" onClick={()=>setRequestsOpen(true)}><UserRoundCheck className="h-5 w-5"/><strong className="mt-2 block">Demandes d’indisponibilité</strong><span className="text-sm text-slate-500">Approuver ou rejeter les demandes des enseignants</span></button><StudyAiAssistant user={user} school={school} year={year} data={studyData}/><StudyPeriodsModule user={user} school={school} year={year} data={studyData}/><StudyRoomsModule user={user} school={school} year={year} data={studyData}/><div className="rounded border border-slate-200 bg-white p-5 shadow-sm"><button type="button" onClick={onLogout} className="secondary-button"><LogOut className="h-4 w-4" /> Déconnexion</button></div></section>}
    ...
    {requestsOpen&&<StudyAvailabilityRequestsDrawer user={user} school={school} year={year} teachers={studyData.teachers} availabilities={studyData.availabilities} timetables={studyData.timetables} entries={studyData.timetableEntries} periods={studyData.periods} subjects={studyData.subjects} classes={studyData.classes} onClose={()=>setRequestsOpen(false)}/>}
