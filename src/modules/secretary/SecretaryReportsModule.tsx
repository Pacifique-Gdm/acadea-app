...
import { ArchiveRestore, Download, FilePlus2, FileText, Trash2 } from "lucide-react";
import { AdminDrawer, SectionTitle } from "../../components/ui";
import { createSecretaryReport, deleteSecretaryReportPermanently, restoreSecretaryReport, subscribeToSecretaryReports, updateSecretaryReport } from "../../services/secretaryReports";
...
export function SecretaryReportsModule({ user, school, year }: { user: AppUser; school: School; year: SchoolYear }) {
  ...
  const readOnly = !canManage || Boolean(selected && selected.status !== "draft");
  ...
  async function confirmPermanentDelete() {
    if (!canManage || !deleteTarget || deleteBusy || deleteConfirmation !== "SUPPRIMER RAPPORT") return;
    setDeleteBusy(true);
    try {
  ...
  async function restoreArchivedReport(report: SecretaryReport) {
    if (!canManage || deleteBusy) return;
    setDeleteBusy(true); setMessage("");
    try {
      await restoreSecretaryReport(user, report);
      setMessage("Rapport restauré.");
  ...
  return <section className="grid gap-4"><SectionTitle title="Rapports" subtitle="Documents administratifs structurés de l'établissement." />{message && <p className="rounded border bg-white p-3 text-sm">{message}</p>}
    <div className="grid min-w-0 gap-2 sm:grid-cols-2 lg:grid-cols-[auto_minmax(0,1fr)_minmax(0,1.2fr)_minmax(0,1fr)_auto]"><button type="button" className="primary-button justify-center whitespace-nowrap px-3" disabled={!canManage} onClick={() => show()}><FilePlus2 className="h-4 w-4" /> Nouveau rapport</button><input className="input min-w-0" placeholder="Rechercher" value={queryText} onChange={(event) => setQueryText(event.target.value)} /><select className="input min-w-0" aria-label="Types de rapport" value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as typeof typeFilter)}><option value="" disabled>Types de rapport</option><option value="all">Tous les types</option>{Object.entries(labels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select className="input min-w-0" aria-label="État d’archivage" value={archiveView} onChange={(event) => setArchiveView(event.target.value as typeof archiveView)}><option value="active">Rapports actifs</option><option value="archived">Archives</option></select><button type="button" className="pdf-export-button whitespace-nowrap px-3" disabled={exportBusy || visible.length === 0} onClick={() => void exportFilteredReports()}><Download className="h-4 w-4" /> {exportBusy ? "Export en cours…" : "Exporter PDF"}</button></div>
    <div className="overflow-x-auto rounded border bg-white"><table className="min-w-[720px] w-full text-sm"><thead className="bg-slate-50 text-left"><tr><th className="p-3">Numéro</th><th>Titre</th><th>Type</th><th>Date</th><th className="text-center">Actions</th></tr></thead><tbody>{visible.map((report) => <tr className="border-t" key={report.id}><td className="p-3 font-semibold">{report.reportNumber}</td><td>{report.title}</td><td>{labels[report.type]}</td><td>{report.documentDate}</td><td><div className="flex items-center justify-center gap-1.5"><SecretaryViewActionButton onClick={() => show(report)} /><button type="button" title="Afficher le PDF" aria-label="Afficher le PDF" disabled={Boolean(pdfBusyId)} className="inline-flex h-9 w-9 items-center justify-center rounded-md bg-slate-100 text-slate-700 transition hover:bg-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 disabled:cursor-wait disabled:opacity-50" onClick={() => void showReportPdf(report)}><FileText className={`h-4 w-4 ${pdfBusyId === report.id ? "animate-pulse" : ""}`} /></button>{report.status === "archived" ? <><button type="button" title="Restaurer" aria-label="Restaurer" disabled={!canManage || deleteBusy} className="inline-flex h-9 w-9 items-center justify-center rounded-md bg-slate-100 text-slate-700 hover:bg-slate-200 disabled:opacity-50" onClick={() => void restoreArchivedReport(report)}><ArchiveRestore className="h-4 w-4" /></button>{report.authorId === user.id && report.archivedFromStatus === "draft" && <button type="button" title="Supprimer" aria-label="Supprimer" disabled={!canManage || deleteBusy} className="inline-flex h-9 w-9 items-center justify-center rounded-md bg-red-50 text-red-700 hover:bg-red-100 disabled:opacity-50" onClick={() => { setDeleteTarget(report); setDeleteConfirmation(""); }}><Trash2 className="h-4 w-4" /></button>}</> : report.status === "draft" && report.authorId === user.id ? <button type="button" title="Supprimer" aria-label="Supprimer" disabled={!canManage || deleteBusy} className="inline-flex h-9 w-9 items-center justify-center rounded-md bg-red-50 text-red-700 transition hover:bg-red-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-700 focus-visible:ring-offset-2 disabled:opacity-50" onClick={() => { setDeleteTarget(report); setDeleteConfirmation(""); }}><Trash2 className="h-4 w-4" /></button> : null}</div></td></tr>)}</tbody></table>{visible.length === 0 && <p className="p-6 text-center text-sm text-slate-500">Aucun rapport correspondant à la recherche et au type sélectionné.</p>}</div>
    {open && <AdminDrawer title={selected ? selected.reportNumber : "Nouveau rapport"} onClose={() => !busy && setOpen(false)} closeLabel="Fermer"><form className="grid gap-3" onSubmit={(event) => { event.preventDefault(); void save(); }}>
      ...
      <div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-1 text-sm font-semibold">Heure de début<input className="input" type="time" value={startTime} disabled={readOnly} onInput={(event) => setStartTime(event.currentTarget.value)} /></label><label className="grid gap-1 text-sm font-semibold">Heure de fin<input className="input" type="time" value={endTime} disabled={readOnly} onInput={(event) => setEndTime(event.currentTarget.value)} /></label></div>
    ...
    {deleteTarget && <SecretaryDocumentDeleteDialog kind="report" value={deleteConfirmation} busy={deleteBusy} onValueChange={setDeleteConfirmation} onCancel={() => { if (!deleteBusy) { setDeleteTarget(null); setDeleteConfirmation(""); } }} onConfirm={() => void confirmPermanentDelete()} />}
  </section>;
}
