    ...
    <div className="grid min-w-0 gap-2 text-sm font-medium text-slate-700">
      ...
      <div className="grid gap-3 rounded border border-slate-200 bg-slate-50 p-3">
        ...
        <div className="flex w-full flex-wrap gap-2">
          ...
          {value && !disabled && (
            <button onClick={() => { setError(""); onValidityChange?.(true); onChange(""); }} className="w-full rounded border border-red-200 bg-white px-3 py-2 text-xs font-bold text-red-600 transition hover:bg-red-50" type="button">
              Supprimer
            </button>
          )}
        </div>
