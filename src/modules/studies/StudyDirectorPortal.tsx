...
import { BarChart3, CalendarClock, History, LogOut, UserRoundCheck } from "lucide-react";
...
import { StudyStudentsModule } from "./StudyStudentsModule";
...
export function StudyDirectorPortal({ user, school, year, renderHeader, renderEnvironmentBanner, onLogout }: { user: AppUser; school: School; year: SchoolYear; renderHeader: () => ReactNode; renderEnvironmentBanner: () => ReactNode; onLogout: () => void }) {
  const [activeTab, setActiveTab] = useState<StudyDirectorTab>("dashboard"), [drawer, setDrawer] = useState<MenuDrawer>(null);
  ...
  return <div className="flex h-screen min-w-0 flex-col overflow-hidden bg-[#f6f8fb]">{renderEnvironmentBanner()}{renderHeader()}<main className="mx-auto w-full max-w-7xl min-w-0 flex-1 overflow-y-auto overflow-x-hidden px-3 py-5 pb-28 sm:px-6 sm:pb-32 lg:px-8">
    {activeTab === "dashboard" && <section className="grid gap-4"><div><h1 className="text-2xl font-bold text-ink">Direction des études</h1><p className="mt-1 break-words text-sm text-slate-600">{school.name} · Année scolaire {year.name}</p></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{dashboardCards.map(([label, value]) => <article key={label} className="flex min-h-32 flex-col justify-between rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</p><p className="mt-3 text-3xl font-bold text-ink">{value}</p></article>)}</div></section>}
    {activeTab === "teachers" && <StudyTeachersModule user={user} school={school} year={year} data={data} />}{activeTab === "students" && <StudyStudentsModule data={data} />}{activeTab === "schedules" && <StudySchedulesModule user={user} school={school} year={year} data={data} />}
    {activeTab === "menu" && <section className="grid gap-3"><StudyAiAssistant user={user} school={school} year={year} data={data} /><button type="button" className={menuButton} onClick={() => setDrawer("age")}><BarChart3 className="h-5 w-5"/><strong className="mt-2 block">Tableau d’homogénéité d’âge</strong></button><button type="button" className={menuButton} onClick={() => setDrawer("requests")}><UserRoundCheck className="h-5 w-5"/><strong className="mt-2 block">Demandes d’indisponibilité</strong></button><button type="button" className={menuButton} onClick={() => setDrawer("history")}><History className="h-5 w-5"/><strong className="mt-2 block">Historique des versions</strong></button><button type="button" className={menuButton} onClick={() => setDrawer("periods")}><CalendarClock className="h-5 w-5"/><strong className="mt-2 block">Périodes & tranches horaires</strong></button><button type="button" onClick={onLogout} className="inline-flex w-full items-center justify-center gap-2 rounded border border-red-200 bg-red-50 px-4 py-3 text-sm font-bold text-red-700 transition hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-red-500"><LogOut className="h-4 w-4"/> Déconnexion</button></section>}
  </main>
  {drawer === "age" && <AgeHomogeneityDrawer open onClose={() => setDrawer(null)} user={user} school={school} year={year} studentSource={data.students} allowedSections={userSectionIds(user)} />}
