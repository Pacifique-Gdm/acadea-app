...
test.describe("Phase 3 — Espace Enseignant", () => {
  ...
  test("provisionne un Enseignant, isole son portail et reçoit une publication sans refresh", async ({ browser }) => {
    ...
    try {
      ...
      const generationStatus = directorPage.getByRole("status");
      await expect(generationStatus).toContainText(/Brouillon version \d+ généré|Impossible de générer l’horaire/, { timeout: 60_000 });
      if ((await generationStatus.textContent())?.includes("Brouillon version")) {
