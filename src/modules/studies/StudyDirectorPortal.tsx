...
import { AlertTriangle, BarChart3, BookOpenCheck, CalendarClock, Clock3, GraduationCap, History, LogOut, MessageSquare, PauseCircle, UserPlus, UserRoundCheck, UsersRound } from "lucide-react";
...
import { PersonnelDrawerContent } from "../menu/PersonnelDrawerContent";
import { StudyTeacherCreateContent } from "./StudyTeacherManagement";

type MenuDrawer = "messages" | "requests" | "history" | "activity-history" | "periods" | "age" | "create-teacher" | "teacher-personnel" | null;
...
export function StudyDirectorPortal({ user, school, year, appData, refreshToken = 0, updateData, createId, formatArchiveDate, renderHeader, renderEnvironmentBanner, onLogout }: { user: AppUser; school: School; year: SchoolYear; appData: AppData; refreshToken?: number; updateData: (next: Partial<AppData>, options?: { persist?: boolean }) => void; createId: (prefix: string) => string; formatArchiveDate: (value?: string) => string; renderHeader: (state: { isRefreshing: boolean; refreshError?: string; onOpenAvailabilityRequests: () => void }) => ReactNode; renderEnvironmentBanner: () => ReactNode; onLogout: () => void }) {
  ...
  return <div className="flex h-screen min-w-0 flex-col overflow-hidden bg-[#f6f8fb]">{renderEnvironmentBanner()}{renderHeader({ isRefreshing: !data.error && (!data.generationReady || !data.schedulesReady), refreshError: data.error, onOpenAvailabilityRequests: () => { setActiveTab("menu"); setDrawer("requests"); } })}<main className="mx-auto w-full max-w-7xl min-w-0 flex-1 overflow-y-auto overflow-x-hidden px-3 py-5 pb-28 sm:px-6 sm:pb-32 lg:px-8">
    ...
    {activeTab === "menu" && <section className="grid gap-3"><StudyAiAssistant user={user} school={school} year={year} data={data} /><button type="button" className={menuButton} onClick={() => setDrawer("messages")}><MessageSquare className="h-5 w-5 shrink-0"/><strong>Message</strong></button><button type="button" className={menuButton} onClick={() => setDrawer("create-teacher")}><UserPlus className="h-5 w-5 shrink-0"/><strong>Créer enseignant</strong></button><button type="button" className={menuButton} onClick={() => setDrawer("teacher-personnel")}><UsersRound className="h-5 w-5 shrink-0"/><strong>Personnels enseignants</strong></button><button type="button" className={menuButton} onClick={() => setDrawer("age")}><BarChart3 className="h-5 w-5 shrink-0"/><strong>Tableau d’homogénéité d’âge</strong></button><button type="button" className={menuButton} onClick={() => setDrawer("requests")}><UserRoundCheck className="h-5 w-5 shrink-0"/><strong>Demandes d’indisponibilité</strong></button><button type="button" className={menuButton} onClick={() => setDrawer("history")}><History className="h-5 w-5 shrink-0"/><strong>Historique des versions</strong></button><button type="button" className={menuButton} onClick={() => setDrawer("periods")}><CalendarClock className="h-5 w-5 shrink-0"/><strong>Périodes & tranches horaires</strong></button><button type="button" className={menuButton} onClick={() => setDrawer("activity-history")}><History className="h-5 w-5 shrink-0"/><strong>Historique</strong></button><button type="button" onClick={onLogout} className="inline-flex w-full items-center justify-center gap-2 rounded border border-red-200 bg-red-50 px-4 py-3 text-sm font-bold text-red-700 transition hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-red-500"><LogOut className="h-4 w-4"/> Déconnexion</button></section>}
  </main>
  {drawer === "messages" && <AdminDrawer title="Message" closeLabel="Fermer la messagerie" onClose={() => setDrawer(null)}><MessagesModule user={user} data={appData} yearData={{ parents: appData.parents, students: data.students, messages: appData.messages }} school={school} year={year} updateData={updateData} createId={createId} /></AdminDrawer>}
  {drawer === "create-teacher" && <AdminDrawer title="Créer enseignant" closeLabel="Fermer la création d’un enseignant" onClose={() => setDrawer(null)}><StudyTeacherCreateContent user={user} school={school} year={year} appData={appData} updateData={updateData}/></AdminDrawer>}
  {drawer === "teacher-personnel" && <AdminDrawer title="Personnels enseignants" closeLabel="Fermer les personnels enseignants" onClose={() => setDrawer(null)}><PersonnelDrawerContent user={user} school={school} allowedRoles={["teacher"]}/></AdminDrawer>}
