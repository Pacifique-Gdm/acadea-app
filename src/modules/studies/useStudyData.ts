...
export function useStudyData(user: AppUser, schoolId: string, schoolYearId: string) {
  ...
  useEffect(() => {
    const unsubscribes = subscribeToStudyData({ user, schoolId, schoolYearId, onTeachers: setTeachers, onSubjects: setSubjects, onClasses: setClasses, onAssignments: setAssignments, onAvailabilities:setAvailabilities,onPeriods:setPeriods,onTimetables:setTimetables,onTimetableEntries:setTimetableEntries,onRooms:setRooms, onError: (cause) => setError(cause.message) });
    return () => unsubscribes.forEach((unsubscribe) => unsubscribe());
  }, [schoolId, schoolYearId, user]);
  return { teachers, subjects, classes, assignments, availabilities, periods, timetables, timetableEntries, rooms, error };
}
