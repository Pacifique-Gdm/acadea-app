...
export function DisciplineStatus({ sanctions, onNewSanction, onCompleteSanction }: DisciplineStatusProps) {
  ...
  return (
    <section className="grid min-w-0 gap-3">
      <div className="flex min-w-0 flex-col gap-3 rounded border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <button onClick={onNewSanction} className="primary-button w-full justify-center sm:w-auto" type="button">
        ...
        {activeSanctions.map((sanction) => (
          <article key={sanction.id} className="min-w-0 rounded border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex min-w-0 flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <h2 className="break-words font-bold text-ink">{sanction.studentName}</h2>
              ...
              <button onClick={() => onCompleteSanction(sanction)} className="secondary-button w-full justify-center lg:w-auto" type="button">
            ...
            <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="rounded bg-slate-50 p-3"><span className="font-semibold text-slate-600">Sanction</span><p className="mt-1 break-words text-ink">{sanction.sanctionType}</p></div>
              ...
              <div className="rounded bg-slate-50 p-3"><span className="font-semibold text-slate-600">Fin prévue</span><p className="mt-1 break-words text-ink">{sanction.expectedEndDate}</p></div>
              <div className="rounded bg-slate-50 p-3"><span className="font-semibold text-slate-600">Statut</span><p className="mt-1 break-words text-ink">{statusLabel(sanction.status)}</p></div>
            </div>
          </article>
        ))}
      </div>
