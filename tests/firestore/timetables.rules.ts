...
beforeAll(async()=>{env=await initializeTestEnvironment({projectId:"demo-timetables",firestore:{rules:readFileSync("firestore.rules","utf8")}})},30000);beforeEach(async()=>{await env.clearFirestore();await seed(`schools/${school}`,{status:"active"});await seed("schools/school-b",{status:"active"});await seed(`schoolYears/${year}`,{schoolId:school,status:"active"});await seed("schoolYears/year-b",{schoolId:"school-b",status:"active"});await seed("users/study_director-user",{id:"study_director-user",role:"study_director",schoolId:school,sectionIds:["Secondaire"]});await seed("teachers/teacher-a",{schoolId:school,schoolYearId:year});await seed("subjects/subject-a",{schoolId:school,schoolYearId:year});await seed("classes/class-a",{schoolId:school,schoolYearId:year});await seed("pedagogicalAssignments/assignment-a",{schoolId:school,schoolYearId:year,teacherId:"teacher-a",subjectId:"subject-a",classId:"class-a"});await seed("schedulePeriods/period-a",{schoolId:school,schoolYearId:year,type:"course"});await seed("students/student-a",{schoolId:school,schoolYearId:year,classId:"class-a",className:"Classe A",section:"Secondaire"})});afterAll(async()=>env?.cleanup(),30000);
...
describe("lecture des classes réellement utilisées",()=>{
  ...
  it("lit CTEB canonique sans élargir le tenant",async()=>{await seed("students/student-cteb",{schoolId:school,schoolYearId:year,className:"7ème CTEB",section:"CTEB"});await seed("users/study_director-user",{id:"study_director-user",role:"study_director",schoolId:school,sectionIds:["CTEB"]});await assertSucceeds(getDoc(doc(db(),"students","student-cteb")));await seed("users/study_director-user",{id:"study_director-user",role:"study_director",schoolId:school,sectionIds:["CETB"]});await assertFails(getDoc(doc(db(),"students","student-cteb")));await assertFails(getDoc(doc(db("study_director","school-b"),"students","student-cteb")))});
  it("conserve le périmètre historique d’un profil sans section",async()=>{await seed("users/study_director-user",{id:"study_director-user",role:"study_director",schoolId:school});await assertSucceeds(getDoc(doc(db(),"students","student-a")))});
