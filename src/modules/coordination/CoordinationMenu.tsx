...
import { BookOpen, CalendarRange, FileClock, Landmark, Network, Settings, UsersRound } from "lucide-react";
import { AdminDrawer } from "../../components/ui";
import type { AppUser, Coordination, School, SubCoordination } from "../../types";
...
export function CoordinationMenu({ coordination, schools, selectedSchoolId, principalCoordinatorName, user, subCoordination }: { coordination: Coordination; schools: School[]; selectedSchoolId: string; principalCoordinatorName: string; user: AppUser; subCoordination?: SubCoordination | null }) {
  ...
  useEffect(() => {
    ...
    loadCoordinationReadModel(coordination.id, schoolIds, user.subCoordinationId).then((result) => { if (!cancelled) setModel(result); }).catch((cause) => { if (!cancelled) setError(cause instanceof Error ? cause.message : "Chargement impossible."); }).finally(() => { if (!cancelled) setLoading(false); });
  ...
  }, [coordination.id, drawer, schoolIds, user.subCoordinationId]);
  ...
  const logs = model.auditLogs.filter((item) => (user.role === "coordination_admin" ? (item.coordinationId === coordination.id || !item.schoolId || schoolIds.includes(item.schoolId)) : (item.subCoordinationId === user.subCoordinationId || Boolean(item.schoolId && schoolIds.includes(item.schoolId)))) && `${item.action} ${item.actorName ?? ""} ${item.details ?? ""}`.toLowerCase().includes(search.toLowerCase())).sort((a, b) => activityTimestamp(b.createdAt) - activityTimestamp(a.createdAt));
