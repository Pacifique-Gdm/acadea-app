...
export class DeterministicTimetableSolver implements TimetableSolver{
  solve(problem:ScheduleProblem,options:TimetableSolverOptions={}):SolverResult{
    ...
    const baselineByAssignment=new Map<string,TimetableEntry[]>();for(const entry of options.baselineEntries??[]){const list=baselineByAssignment.get(entry.assignmentId)??[];list.push(entry);baselineByAssignment.set(entry.assignmentId,list)}for(const entries of baselineByAssignment.values())entries.sort((a,b)=>`${a.dayOfWeek}-${a.periodId}`.localeCompare(`${b.dayOfWeek}-${b.periodId}`));
    const fixedEntries=options.fixedEntries??[],activeById=new Map(active.map(assignment=>[assignment.id,assignment])),fixedByAssignment=new Map<string,TimetableEntry[]>();
    for(const entry of fixedEntries){const assignment=activeById.get(entry.assignmentId);if(!assignment||entry.schoolId!==problem.schoolId||entry.schoolYearId!==problem.schoolYearId||entry.teacherId!==assignment.teacherId||entry.classId!==assignment.classId||entry.subjectId!==assignment.subjectId){return{success:false,entries:[],failures:[{assignmentId:entry.assignmentId,teacherId:entry.teacherId,classId:entry.classId,subjectId:entry.subjectId,required:0,availableCapacity:0,reason:"Une séance verrouillée ne correspond plus à une affectation active."}],statistics:{exploredBranches,durationMs:Date.now()-started,timedOut}};}const list=fixedByAssignment.get(entry.assignmentId)??[];list.push(entry);fixedByAssignment.set(entry.assignmentId,list)}
    const invalidFixed=jobs.find(job=>{const count=fixedByAssignment.get(job.assignment.id)?.length??0;return count>job.assignment.weeklyPeriods||count%(job.blockSize??1)!==0});if(invalidFixed)return{success:false,entries:[],failures:[{assignmentId:invalidFixed.assignment.id,teacherId:invalidFixed.assignment.teacherId,classId:invalidFixed.assignment.classId,subjectId:invalidFixed.assignment.subjectId,required:invalidFixed.assignment.weeklyPeriods,availableCapacity:fixedByAssignment.get(invalidFixed.assignment.id)?.length??0,reason:"Le nombre de séances verrouillées est incompatible avec le volume actuel."}],statistics:{exploredBranches,durationMs:Date.now()-started,timedOut}};
    ...
    const candidateCost=(assignment:PedagogicalAssignment,index:number,candidate:Candidate)=>{const prior=baselineByAssignment.get(assignment.id)?.[index*(assignment.blockSize??1)];let cost=prior?(prior.dayOfWeek===candidate.day?(prior.periodId===candidate.periodIds[0]?0:1):4):2;if(morning.has(assignment.id)&&candidate.periodIds.some(id=>(periodOrder.get(id)??0)>morningLimit))cost+=3;return cost};
    const remainingBlocks=new Map(jobs.map(job=>[job.assignment.id,job.blocks-(fixedByAssignment.get(job.assignment.id)?.length??0)/job.blockSize])),totalBlocks=[...remainingBlocks.values()].reduce((total,blocks)=>total+blocks,0);
    ...
    const canPlace=(assignment:PedagogicalAssignment,candidate:Candidate)=>{const countKey=`${assignment.id}|${candidate.day}`;if((daily.get(countKey)||0)+candidate.periodIds.length>(problem.maxSameAssignmentPeriodsPerDay??DEFAULT_MAX_SAME_ASSIGNMENT_PERIODS_PER_DAY))return false;return!candidate.periodIds.some(periodId=>teacherBusy.has(`${assignment.teacherId}|${candidate.day}|${periodId}`)||studentConflict(assignment,candidate.day,periodId)||(assignment.preferredRoomId&&roomBusy.has(`${assignment.preferredRoomId}|${candidate.day}|${periodId}`)))};
    for(const entry of fixedEntries){const assignment=activeById.get(entry.assignmentId)!;const countKey=`${assignment.id}|${entry.dayOfWeek}`,teacherKey=`${assignment.teacherId}|${entry.dayOfWeek}|${entry.periodId}`,roomKey=assignment.preferredRoomId?`${assignment.preferredRoomId}|${entry.dayOfWeek}|${entry.periodId}`:"";if(teacherBusy.has(teacherKey)||studentConflict(assignment,entry.dayOfWeek,entry.periodId)||(roomKey&&roomBusy.has(roomKey))){return{success:false,entries:[],failures:[{assignmentId:assignment.id,teacherId:assignment.teacherId,classId:assignment.classId,subjectId:assignment.subjectId,required:assignment.weeklyPeriods,availableCapacity:0,reason:"Deux séances verrouillées utilisent le même créneau."}],statistics:{exploredBranches,durationMs:Date.now()-started,timedOut}};}teacherBusy.add(teacherKey);if(roomKey)roomBusy.add(roomKey);const slot=`${entry.dayOfWeek}|${entry.periodId}`;studentBusy.set(slot,[...(studentBusy.get(slot)??[]),assignment]);daily.set(countKey,(daily.get(countKey)??0)+1)}
    const search=(position:number):boolean=>{if(position===totalBlocks)return true;if(++exploredBranches>maxBranches||Date.now()>deadline){timedOut=true;return false;}const choices=jobs.filter(job=>(remainingBlocks.get(job.assignment.id)??0)>0).map(job=>{const remaining=remainingBlocks.get(job.assignment.id)??0;return{job,remaining,candidates:job.candidates.filter(candidate=>canPlace(job.assignment,candidate))}}).sort((a,b)=>(a.candidates.length-a.remaining)-(b.candidates.length-b.remaining)||a.candidates.length-b.candidates.length||b.job.assignment.weeklyPeriods-a.job.assignment.weeklyPeriods||a.job.assignment.id.localeCompare(b.job.assignment.id));const choice=choices[0];if(!choice||!choice.candidates.length)return false;const task={assignment:choice.job.assignment,index:choice.job.blocks-choice.remaining,candidates:[...choice.candidates].sort((a,b)=>candidateCost(choice.job.assignment,choice.job.blocks-choice.remaining,a)-candidateCost(choice.job.assignment,choice.job.blocks-choice.remaining,b)||`${a.day}-${a.periodIds.join()}`.localeCompare(`${b.day}-${b.periodIds.join()}`))};for(const candidate of task.candidates){const countKey=`${task.assignment.id}|${candidate.day}`;candidate.periodIds.forEach(periodId=>{teacherBusy.add(`${task.assignment.teacherId}|${candidate.day}|${periodId}`);const key=`${candidate.day}|${periodId}`;studentBusy.set(key,[...(studentBusy.get(key)??[]),task.assignment]);if(task.assignment.preferredRoomId)roomBusy.add(`${task.assignment.preferredRoomId}|${candidate.day}|${periodId}`)});daily.set(countKey,(daily.get(countKey)||0)+candidate.periodIds.length);remainingBlocks.set(task.assignment.id,choice.remaining-1);selected.push({assignment:task.assignment,index:task.index,candidate});if(search(position+1))return true;selected.pop();remainingBlocks.set(task.assignment.id,choice.remaining);daily.set(countKey,(daily.get(countKey)||0)-candidate.periodIds.length);candidate.periodIds.forEach(periodId=>{teacherBusy.delete(`${task.assignment.teacherId}|${candidate.day}|${periodId}`);const key=`${candidate.day}|${periodId}`;const occupied=studentBusy.get(key)??[];const removal=occupied.lastIndexOf(task.assignment);if(removal>=0)occupied.splice(removal,1);if(occupied.length)studentBusy.set(key,occupied);else studentBusy.delete(key);if(task.assignment.preferredRoomId)roomBusy.delete(`${task.assignment.preferredRoomId}|${candidate.day}|${periodId}`)});if(timedOut)return false;}return false;};
