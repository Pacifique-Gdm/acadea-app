...
function upload(name: string, type: string, size = 1024, overrides: Record<string, string> = {}) {
  return context().storage().ref(`message-uploads/${schoolId}/${senderId}/${draftId}/${name}`).put(new Uint8Array(size), { contentType: type, customMetadata: { schoolId, schoolYearId: yearId, senderId, draftId, originalName: "document.pdf", ...overrides } });
}
...
describe("pièces jointes de messagerie", () => {
  ...
  it("refuse un objet de plus de 10 Mo", async () => { await assertFails(upload(`${fileId}.pdf`, "application/pdf", 10 * 1024 * 1024 + 1)); });
});
