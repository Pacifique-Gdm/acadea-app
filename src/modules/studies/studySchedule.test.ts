...
describe("continuité pédagogique",()=>{it("accepte uniquement des périodes jointives du même modèle",()=>{const periods=[p("p1","08:00","09:00","course",1),p("p2","09:00","10:00","course",2),p("p3","10:15","11:15","course",3)];expect(arePeriodsPedagogicallyConsecutive(periods.slice(0,2),periods)).toBe(true);expect(arePeriodsPedagogicallyConsecutive(periods,periods)).toBe(false);expect(maxConsecutiveCoursePeriods(periods)).toBe(2)});it.each(["break","recess"] as const)("traverse une tranche officielle %s sans la compter",type=>{const periods=[p("p1","08:00","09:00","course",1),p("pause","09:00","09:15",type,2),p("p2","09:15","10:15","course",3)];expect(arePeriodsPedagogicallyConsecutive([periods[0],periods[2]],periods)).toBe(true);expect(maxConsecutiveCoursePeriods(periods)).toBe(2)});it("traverse plusieurs pauses officielles jointives",()=>{const periods=[p("p1","08:00","09:00","course",1),p("pause-1","09:00","09:15","break",2),p("p2","09:15","10:15","course",3),p("pause-2","10:15","10:30","recess",4),p("p3","10:30","11:30","course",5)];expect(arePeriodsPedagogicallyConsecutive([periods[0],periods[2],periods[4]],periods)).toBe(true);expect(maxConsecutiveCoursePeriods(periods)).toBe(3)});it("ne traverse ni un trou horaire ni une tranche inactive",()=>{const gap=[p("p1","08:00","09:00","course",1),p("p2","09:15","10:15","course",2)];expect(arePeriodsPedagogicallyConsecutive(gap,gap)).toBe(false);const inactive={...p("pause","09:00","09:15","break",2),active:false};const periods=[p("p1","08:00","09:00","course",1),inactive,p("p2","09:15","10:15","course",3)];expect(arePeriodsPedagogicallyConsecutive([periods[0],periods[2]],periods)).toBe(false)});});
