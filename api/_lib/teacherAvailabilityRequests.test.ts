...
describe("reviewAvailabilityRequest", () => {
  ...
  it("rejette sans créer de disponibilité", async () => { const db = fakeDb(base); const result = await reviewAvailabilityRequest({ db, caller: { uid: "d", role: "study_director", schoolId: "s" }, requestId: "r", action: "REJECT", reviewComment: "Impossible" }); expect(result.status).toBe("REJECTED"); expect(db.writes).toHaveLength(1); });
