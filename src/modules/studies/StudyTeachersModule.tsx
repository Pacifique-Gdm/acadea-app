...
import { createStudySubject, createStudyTeacher, savePedagogicalAssignment, setPedagogicalAssignmentActive } from "./studyService";
...
export function StudyTeachersModule({ user, school, year, data }: { user: AppUser; school: School; year: SchoolYear; data: ReturnType<typeof useStudyData> }) {
  ...
  const teacherRows = useMemo(() => teachers.map((teacher) => {
  ...
  async function submitTeacher() {
    setBusy(true); setFeedback("");
    try { await createStudyTeacher({ user, schoolId: school.id, schoolYearId: year.id, firstName, lastName }); setTeacherOpen(false); setFirstName(""); setLastName(""); }
  ...
  return <section className="grid gap-4">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-bold text-ink">Enseignants</h1><p className="text-sm text-slate-600">Gestion des enseignants et de leurs affectations pédagogiques.</p></div><div className="flex flex-wrap gap-2"><button className="secondary-button" type="button" onClick={() => { setFeedback(""); setTeacherOpen(true); }}><UserRound className="h-4 w-4" /> Ajouter un enseignant</button><button className="primary-button" type="button" onClick={() => openAssignment()}><Plus className="h-4 w-4" /> Ajouter une affectation</button></div></div>
    ...
    {selectedTeacher && <AdminDrawer title={`Fiche pédagogique — ${selectedTeacher.fullName}`} closeLabel="Fermer la fiche pédagogique" onClose={() => setSelectedTeacher(undefined)}><p className="rounded bg-slate-50 p-3 text-sm"><strong>École :</strong> {school.name}<br/><strong>Année scolaire :</strong> {year.name}<br/><strong>Statut :</strong> {selectedTeacher.status === "active" ? "Actif" : "Inactif"}</p><div className="flex justify-between gap-2"><h3 className="font-bold text-ink">Affectations</h3><button type="button" className="primary-button" onClick={() => openAssignment(undefined, selectedTeacher)}><Plus className="h-4 w-4" /> Ajouter</button></div>{assignments.filter((item) => item.teacherId === selectedTeacher.id).map((item) => <article key={item.id} className="rounded border border-slate-200 p-3 text-sm"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{subjects.find((subject) => subject.id === item.subjectId)?.name ?? "Matière inconnue"}</p><p className="text-slate-600">{classes.find((schoolClass) => schoolClass.id === item.classId)?.name ?? "Classe inconnue"} · {item.weeklyPeriods} périodes/semaine · {item.active ? "Active" : "Inactive"}</p></div><button type="button" aria-label="Modifier l’affectation" className="inline-flex h-9 w-9 items-center justify-center rounded bg-slate-100" onClick={() => openAssignment(item)}><Pencil className="h-4 w-4" /></button></div>{item.active && <button type="button" className="mt-2 text-xs font-semibold text-red-700" onClick={() => void setPedagogicalAssignmentActive(user, item, false)}>Désactiver</button>}</article>)}<div className="grid gap-2"><div className="flex items-center justify-between gap-2"><h3 className="font-bold text-ink">Disponibilités</h3><button type="button" className="primary-button" onClick={() => setAvailabilityTeacher(selectedTeacher)}>Configurer</button></div><TeacherAvailabilitySummary teacherId={selectedTeacher.id} items={data.availabilities} /></div><p className="text-right font-bold">Charge totale : {teacherWorkload(selectedTeacher.id, assignments)} périodes/semaine</p></AdminDrawer>}

    {teacherOpen && <AdminDrawer title="Ajouter un enseignant" closeLabel="Fermer le formulaire enseignant" onClose={() => !busy && setTeacherOpen(false)}><label className="grid gap-1 text-sm font-semibold">Prénom<input className="input" value={firstName} onChange={(event) => setFirstName(event.target.value)} /></label><label className="grid gap-1 text-sm font-semibold">Nom<input className="input" value={lastName} onChange={(event) => setLastName(event.target.value)} /></label>{feedback && <p role="alert" className="text-sm text-red-700">{feedback}</p>}<div className="grid grid-cols-2 gap-2"><button type="button" className="secondary-button justify-center" disabled={busy} onClick={() => setTeacherOpen(false)}>Annuler</button><button type="button" className="primary-button justify-center" disabled={busy} onClick={() => void submitTeacher()}>{busy ? "Enregistrement…" : "Enregistrer"}</button></div></AdminDrawer>}
