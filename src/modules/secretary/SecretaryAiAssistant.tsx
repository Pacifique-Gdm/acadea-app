...
import { editedReportSectionsToApply, getTargetSections } from "./reportAiSections";
...
export function SecretaryAiAssistant({ user, schoolId, academicYearId, documentId, documentType, documentCategory, documentTypeLabel, documentDate, documentTime, documentEndTime, schoolName, academicYearName, sections, sectionLabels = {}, initialSection, label = "Générer avec l’Assistant IA", aiAssistant, onAccept, onApplySections }: {
  ...
  const [action, setAction] = useState<AiAction>("correct"); const [tone, setTone] = useState<AiTone>("administrative"); const [length, setLength] = useState<AiLength>("standard"); const [instruction, setInstruction] = useState("");
  ...
  async function run() {
    if (lock.current || limitReached) return; if (!consent) { setError("Confirmez que la proposition sera vérifiée avant utilisation."); return; }
    ...
    try {
      ...
      if (import.meta.env.VITE_APP_ENV !== "production") console.info("Secretary AI report request", { action, scope: effectiveScope, tone, length, hasAdditionalInstruction: instruction.trim().length > 0, sectionKeysSent: Object.keys(sectionsSent) });
      const response = await requestSecretaryAi(user, {
        ...
        ...(effectiveScope !== "full_document" ? { targetSection: { key: effectiveScope, value: sectionsSent[effectiveScope] ?? "" } } : {}), action, originalText: original, context: sections, tone, length, additionalInstruction: instruction, consentConfirmed: true,
      });
      const normalizedSections = Object.fromEntries(Object.keys(sections).map((key) => [key, response.sections?.[key] ?? sections[key] ?? ""]));
      if (import.meta.env.VITE_APP_ENV !== "production") console.info("Secretary AI report response", { sectionKeysReceived: Object.keys(response.sections ?? {}), generatedSectionKeys: Object.keys(normalizedSections) });
  ...
  return <div className="grid justify-items-end gap-2"><aside className="w-full max-w-xl rounded border border-violet-200 bg-violet-50 p-4 text-sm text-slate-700"><p className="font-bold text-ink">Assistant IA</p><p className="mt-2 font-semibold">Quota mensuel</p><p>{monthlyUsage} / {monthlyLimit} utilisations</p>{limitReached ? <p role="alert" className="mt-3 font-semibold text-amber-800">Le quota mensuel de votre établissement est atteint.<br />Veuillez contacter votre administrateur ou réessayer le mois prochain.</p> : <p className="mt-2">Il vous reste : <strong>{remaining} utilisations ce mois.</strong></p>}<p className="mt-2 text-xs text-slate-500">Le quota est réinitialisé automatiquement au début de chaque mois.</p></aside><button type="button" className="secondary-button" disabled={usageLoading || limitReached} onClick={() => { setScope("full_document"); setSection(initialSection ?? (documentCategory === "rapport" ? Object.keys(sections)[0] ?? "" : "")); setOpen(true); }}><Bot className="h-4 w-4" /> {label}</button>{open && <AdminDrawer title="Assistant IA" onClose={() => !busy && setOpen(false)} closeLabel="Fermer"><div className="grid gap-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-1 text-sm">Ton<select className="input" value={tone} onChange={(event) => { setTone(event.target.value as AiTone); invalidateResult(); }}><option value="neutral">Neutre</option><option value="administrative">Administratif</option><option value="formal">Formel</option><option value="professional">Professionnel</option><option value="concise">Concis</option><option value="diplomatic">Diplomatique</option></select></label><label className="grid gap-1 text-sm">Longueur<select className="input" value={length} onChange={(event) => { setLength(event.target.value as AiLength); invalidateResult(); }}><option value="short">Courte</option><option value="standard">Standard</option><option value="developed">Plus développée</option></select></label></div>
    <label className="grid gap-1 text-sm">Instruction complémentaire<textarea className="input min-h-20" value={instruction} onChange={(event) => { setInstruction(event.target.value); invalidateResult(); }} /></label>
