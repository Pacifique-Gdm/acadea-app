...
export function CoordinationMenu({ coordination, schools, selectedSchoolId, principalCoordinatorName, user, subCoordination, onLogout }: { coordination: Coordination; schools: School[]; selectedSchoolId: string; principalCoordinatorName: string; user: AppUser; subCoordination?: SubCoordination | null; onLogout: () => void }) {
  ...
  function drawerContent(): ReactNode {
    if (drawer === "fees") return <><SchoolPdfControls schools={schools} schoolId={drawerSchoolId} user={user} onSchoolChange={setDrawerSchoolId} onExport={() => void exportPdf("fees")}/>{financialGroups.map((group) => <section key={group.currency} className="grid gap-2 rounded border border-slate-200 p-3"><h3 className="font-bold text-ink">Types de frais — {group.currency}</h3><ReadTable headers={["Type", "Attendu", "Payé", "Solde", "%"]} rows={group.feeProgressRows.map((item) => [item.name, formatCurrencyMoney(item.expected, group.currency), formatCurrencyMoney(item.paid, group.currency), formatCurrencyMoney(item.remaining, group.currency), `${item.rate}%`])} /></section>)}</>;
    if (drawer === "finance") return <><SchoolPdfControls schools={schools} schoolId={drawerSchoolId} user={user} onSchoolChange={setDrawerSchoolId} onExport={() => void exportPdf("finance")}/><div className="grid gap-2 sm:grid-cols-2"><input className="input min-w-0 w-full" aria-label="Date de début" type="date" value={from} onChange={(event) => setFrom(event.target.value)} /><input className="input min-w-0 w-full" aria-label="Date de fin" type="date" value={to} onChange={(event) => setTo(event.target.value)} /></div>{financialGroups.map((group) => <section key={group.currency} className="grid gap-3 rounded border border-slate-200 p-3"><h3 className="font-bold text-ink">Synthèse financière — {group.currency}</h3><ReadTable headers={["Indicateur", "Montant"]} rows={[["Attendu", formatCurrencyMoney(group.expected, group.currency)], ["Encaissé", formatCurrencyMoney(group.paid, group.currency)], ["Dépenses", formatCurrencyMoney(group.expenses, group.currency)], ["Reste", formatCurrencyMoney(group.remaining, group.currency)], ["Recouvrement", `${group.recoveryRate}%`]]} /><h4 className="font-semibold text-ink">Paiements — {group.currency}</h4><ReadTable headers={["École", "Date", "Montant"]} rows={group.payments.map((item) => [schoolName(item.schoolId), item.paidAt, formatCurrencyMoney(item.amount, group.currency)])} /><h4 className="font-semibold text-ink">Dépenses — {group.currency}</h4><ReadTable headers={["École", "Date", "Montant"]} rows={group.expenseRows.map((item) => [schoolName(item.schoolId), item.spentAt, formatCurrencyMoney(item.amount, group.currency)])} /></section>)}</>;
    ...
    if (drawer === "years") return <><p className="rounded bg-blue-50 p-3 text-sm">Année de référence : <b>{referenceYear ?? "Non définie"}</b></p><button className="pdf-export-button w-full sm:w-auto" type="button" onClick={() => void exportPdf("years")}>Exporter PDF</button><ReadTable headers={["École", "Année active", "Alignement"]} rows={visibleYearRows.map((item) => [item.schoolName, item.activeYear?.name ?? "Aucune", !referenceYear || item.activeYear?.name === referenceYear ? "Alignée" : "Année non alignée"])} />{yearResults.length > 0 && <ReadTable headers={["École", "Résultat", "Motif"]} rows={yearResults.map((item) => [schoolName(item.schoolId), item.status, item.reason ?? "—"])} />}{user.role === "coordination_admin" ? <><button className="secondary-button w-full justify-center" disabled={yearBusy} type="button" onClick={() => void mutateYears("close")}>Clôturer les années prêtes</button><div className="grid min-w-0 gap-2 rounded border p-3"><input className="input min-w-0 w-full" placeholder="2027-2028" value={yearName} onChange={(event) => setYearName(event.target.value)} /><input className="input min-w-0 w-full" type="date" value={startsAt} onChange={(event) => setStartsAt(event.target.value)} /><input className="input min-w-0 w-full" type="date" value={endsAt} onChange={(event) => setEndsAt(event.target.value)} /><button className="primary-button w-full justify-center" disabled={yearBusy || !yearName || !startsAt || !endsAt} type="button" onClick={() => void mutateYears("open")}>Ouvrir la nouvelle année</button></div></> : <p className="rounded border border-blue-100 bg-blue-50 p-3 text-sm">Consultation uniquement. L’ouverture et la clôture restent réservées au Coordinateur principal.</p>}</>;
