...
export function useStudyData(user: AppUser, schoolId: string, schoolYearId: string, refreshToken = 0) {
  ...
  useEffect(()=>{const unsubscribes=subscribeToStudyData({user,schoolId,schoolYearId,onTeachers:setTeachers,onSubjects:setSubjects,onClasses:setClasses,onStudents:setStudents,onAssignments:setAssignments,onTitulars:setTitulars,onAvailabilities:setAvailabilities,onPeriods:setPeriods,onTimetables:setTimetables,onTimetableEntries:setTimetableEntries,onRooms:setRooms,onAttendanceSettings:(items)=>setAttendanceSettings(items[0]),onError:(cause)=>setError(cause.message)});return()=>unsubscribes.forEach(unsubscribe=>unsubscribe());},[refreshToken,schoolId,schoolYearId,user]);
  ...
  return {teachers:scopedTeachers,subjects:scopedSubjects,classes:scopedClasses,sourceClasses:classes,students:scopedStudents,assignments:scopedAssignments,titulars:scopedTitulars,availabilities,periods,timetables,timetableEntries,rooms,attendanceSettings,error};
}
