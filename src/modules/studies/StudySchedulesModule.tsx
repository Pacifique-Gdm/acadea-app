...
import { currentStudyTimetable, getActiveCoursePeriods, schedulePeriodLabel, studyDayLabel } from "./studySchedule";
...
const viewLabels: Record<View, string> = { class: "Par classe", teacher: "Par enseignant" };
...
export function StudySchedulesModule({ user, school, year, data }: { user: AppUser; school: School; year: SchoolYear; data: ReturnType<typeof useStudyData> }) {
  ...
  const filtered = entries.filter((item) => view === "class" ? (!selected || (() => { const assignment = personnel.assignments.find((candidate) => candidate.id === item.assignmentId); const schoolClass = data.classes.find((candidate) => candidate.id === selected); return Boolean(assignment && schoolClass && assignmentAppliesToClass(assignment, schoolClass, scheduleClasses)); })()) : (!selected || item.teacherId === selected));
