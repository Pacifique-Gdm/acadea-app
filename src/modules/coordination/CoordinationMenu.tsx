...
export function CoordinationMenu({ coordination, schools, selectedSchoolId }: { coordination: Coordination; schools: School[]; selectedSchoolId: string }) {
  ...
  const personnel = model.personnel.filter((item) => schoolIds.includes(item.schoolId ?? "") && `${item.name} ${item.email ?? ""} ${item.role}`.toLowerCase().includes(search.toLowerCase()));
